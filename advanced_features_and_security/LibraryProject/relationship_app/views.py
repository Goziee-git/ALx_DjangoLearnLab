...
def register(request):
    if request.method == 'POST':
        form = CustomUserCreationForm(request.POST, request.FILES)  # Handle file uploads for profile_photo
        if form.is_valid():
            user = form.save()
            # Assign role-based permissions (if roles are still part of the logic)
            #if hasattr(user, 'userprofile') and user.userprofile.role == 'Admin':
            if user.role == 'Admin':
                assign_admin_permissions(user)
            #elif hasattr(user, 'userprofile') and user.userprofile.role == 'Member':
            elif user.role == 'Member':
                assign_member_permissions(user)
            #elif hasattr(user, 'userprofile') and user.userprofile.role == 'Librarian':
